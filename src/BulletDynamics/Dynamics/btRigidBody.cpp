...
void btRigidBody::setMassProps(btScalar mass, const btVector3& inertia)
{
	...
	{
		backupMassProps();
		setupMassAtImpact(m_massImpactData.LowerMass, m_massImpactData.UpperMass, m_massImpactData.LowerLimitDistance, m_massImpactData.UpperLimitDistance);
	}
}
...
{
	m_massImpactData.LowerMass = btMax(m_massImpactData.MassBackup, lowerMass);
	m_massImpactData.UpperMass = btMax(upperMass, m_massImpactData.LowerMass);
...
void btRigidBody::activeMassAtImpact(bool isActive)
{
	...
	if (m_massImpactData.IsActive)
	{
		// On activation make sure to sync backup and tweaked masses
		backupMassProps();
		setupMassAtImpact(m_massImpactData.LowerMass, m_massImpactData.UpperMass, m_massImpactData.LowerLimitDistance, m_massImpactData.UpperLimitDistance);
	}
	else
