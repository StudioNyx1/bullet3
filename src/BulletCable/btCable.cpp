...
void btCable::PrepareSolver()
{
	...
	for (i = 0, ni = this->m_anchors.size(); i < ni; ++i)
	{
		...
		//// TEST(jeremy) Uncomment to disable MassAtImpact in softbody
		//// NOTE(jeremy) Tension will likely be incorrect if used
		//// Always use body's original mass properties for anchor constraint
		//if (!b->isStaticOrKinematicObject() && b->IsMassAtImpactActive())
		//{
		//	b->restoreMassProps();
		//}
		//// TEST(jeremy) Uncomment to disable MassAtImpact in softbody
